...
Client.prototype.starttls = function starttls (options,
  ...
  function onSend (sendErr, emitter) {
    ...
    emitter.on('end', function (_res) {
      ...
      // CF Workers: sock.startTls()로 기존 소켓을 TLS로 업그레이드한다.
      // startTls()는 내부 상태를 교체하고 준비 완료 시 'connect'를 발생시킨다.
      const secure = sock.startTls(options)
      secure.once('connect', function () {
...
Client.prototype.connect = function connect () {
  ...
  function connectSocket (cb) {
    ...
    socket = createSocket({
      hostname: host,
      port,
      socketPath: !server ? self.socketPath : undefined,
      secure: !!(server && server.secure),
  ...
  function initSocket (server) {
    ...
    // CF Workers 소켓은 setKeepAlive를 지원하지 않는다 (CfSocket에서 no-op 처리됨).
